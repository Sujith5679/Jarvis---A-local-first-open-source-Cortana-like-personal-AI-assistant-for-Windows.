...
voice model is found, `synthesize()` raises `SynthesisError` so callers can
fall back to text-only output rather than crashing (spec.md §32-style
graceful degradation — voice is optional, text always works).
"""
